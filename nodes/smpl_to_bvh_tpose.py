"""
SMPLtoBVH T-Pose Fix - Convert SMPL motion data to BVH with TRUE T-POSE offsets

This variant fixes the arm skeleton offsets to create a proper T-pose where arms extend
horizontally instead of hanging vertically down. SMPL rotations are relative to T-pose,
so the skeleton offsets MUST create T-pose for correct motion.

CHANGES FROM ORIGINAL:
- L_Elbow: [0.0, -0.25, 0.0] → [-0.25, 0.0, 0.0]  (horizontal left, not down!)
- R_Elbow: [0.0, -0.25, 0.0] → [0.25, 0.0, 0.0]   (horizontal right, not down!)
- L_Wrist: [0.0, -0.25, 0.0] → [-0.25, 0.0, 0.0]  (horizontal left)
- R_Wrist: [0.0, -0.25, 0.0] → [0.25, 0.0, 0.0]   (horizontal right)
"""
...
class SMPLtoBVH_TPose:
    """
    Convert SMPL motion parameters to BVH with TRUE T-POSE skeleton offsets.

    This fixes the arm orientation issue where the original offsets created an
    arms-down pose instead of T-pose, causing SMPL rotations (which are relative
    to T-pose) to be applied incorrectly.
    """
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "output_path": ("STRING", {
                    "default": "output/motion_tpose.bvh",
                    "multiline": False,
                }),
                "fps": ("INT", {
                    "default": 30,
    ...
    def convert_to_bvh(
        ...
        try:
            ...
            if len(body_pose.shape) == 2:
                ...
                Log.info(f"[SMPLtoBVH T-Pose] Detected {num_body_joints}-joint SMPL variant ({num_params} parameters)")
            ...
            if num_body_joints == 21:
                ...
                Log.info("[SMPLtoBVH T-Pose] Using 21-joint skeleton (GVHMR variant, no hands)")
            elif num_body_joints == 23:
                joint_names = SMPL_23_JOINT_NAMES
                parent_indices = SMPL_23_PARENTS
                Log.info("[SMPLtoBVH T-Pose] Using 23-joint skeleton (full SMPL with hands)")
            else:
            ...
            Log.info(f"[SMPLtoBVH T-Pose] Rotation ranges (degrees):")
            ...
            info = (
                ...
                f"Fix: TRUE T-POSE offsets (arms horizontal)\n"
            )

            Log.info(f"[SMPLtoBVH T-Pose] Converted {num_frames} frames to {output_path}")
            return (bvh_data, str(output_path.absolute()), info)

        except Exception as e:
            error_msg = f"SMPLtoBVH T-Pose failed: {str(e)}"
            Log.error(error_msg)
            import traceback
            traceback.print_exc()
            return ({}, "", error_msg)
    ...
    def _write_joint(self, lines: list, joint_idx: int, indent_level: int, scale: float):
        ...
        offset = SMPL_OFFSETS_TPOSE.get(joint_idx, [0.0, 0.0, 0.0])  # Use T-POSE offsets
